...
# ----------------------------------------------------------------------
# 2. Gemini AI 함수 정의: 검색 및 구조화 (출처 링크 요청 포함)
# ----------------------------------------------------------------------
...
def get_austin_jobs_from_gemini(job_type: str):
    ...
    # 필수 항목에 '출처 링크'를 추가합니다.
    details = "임금(시급/연봉), 자격조건, 신입채용여부, 시작시점, 재택여부, 파트타임여부, 계약조건, **출처 링크**"
    ...
    **출처 링크는 반드시 클릭 가능한 마크다운 하이퍼링크 형식으로 제공해야 합니다.**
...
if 'job_results' in st.session_state:
    ...
    # Gemini가 생성한 마크다운 테이블을 직접 출력 (링크 포함)
    st.markdown(st.session_state['job_results'])
    
    st.caption("제공된 정보는 Gemini AI의 실시간 웹 검색 결과이며, **출처 링크**를 통해 최종적인 채용 공고 원본을 반드시 확인해야 합니다.")
